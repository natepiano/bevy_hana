...
///
/// The same panel produces the same fingerprint on every launch. That is what makes it safe to
...
/// Kernel-facing identity evidence retained for one enumerated display panel.
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) enum PanelIdentityEvidence {
    /// Serial value the panel published in its EDID.
    #[cfg(any(test, target_os = "windows", all(unix, not(target_os = "macos"))))]
    ReportedSerial(ReportedId),
    /// Stable descriptor material that identifies only a restore target.
    Synthesized {
        /// Digest material retained from the panel descriptor.
        fingerprint: PanelFingerprint,
        /// Why this observation did not include a panel-published serial.
        serial:      ReportedSerial,
    },
    /// This scan produced no durable panel identity.
    Unavailable {
...
pub(super) enum MonitorIdentificationError {
    ...
    /// This platform cannot expose stable panel evidence through the active API.
...
pub(super) enum OperatingSystemQueryError {
    ...
    /// The platform property used for durable panel evidence could not be read.
    #[cfg(any(test, target_os = "windows", all(unix, not(target_os = "macos"))))]
...
///
/// The reporter evidence is the only input that can mint a `DeviceKey`. `PanelIdentity` remains
/// here solely because phase 19 must decode existing v4 files before converting them to the
/// reporter's exact key.
pub(super) fn classify_panel_evidence(
    evidence: &native::QualifiedEvidence,
) -> (PanelIdentityEvidence, PanelIdentity) {
    let fingerprint = PanelFingerprint::from_evidence_bytes(evidence.stable_bytes());
    let legacy = PanelIdentity::Fingerprinted(fingerprint);
    let reported = match evidence {
        #[cfg(any(test, target_os = "windows"))]
        native::QualifiedEvidence::WindowsEdid(EdidIdentityEvidence::ReportedSerial(serial)) => {
            PanelIdentityEvidence::ReportedSerial(serial.clone())
        },
        #[cfg(any(test, target_os = "windows"))]
        ...
        native::QualifiedEvidence::InternalConnector(_) => PanelIdentityEvidence::Synthesized {
            fingerprint,
            serial: ReportedSerial::PlatformCannotReport,
        },
        #[cfg(target_os = "macos")]
        native::QualifiedEvidence::MacOsDisplayUuid(_) => PanelIdentityEvidence::Synthesized {
            fingerprint,
            serial: ReportedSerial::PlatformCannotReport,
        },
        #[cfg(test)]
...
#[cfg(any(test, target_os = "windows", all(unix, not(target_os = "macos"))))]
const fn synthesized_without_serial(fingerprint: PanelFingerprint) -> PanelIdentityEvidence {
