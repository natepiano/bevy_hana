...
/// Frozen v4 panel identity retained only for decoding old files and unresolved v5 targets.
///
...
/// minting a stronger key than the fresh reporter scan supports.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Serialize, Deserialize)]
pub(crate) enum PersistedPanelIdentityV4 {
    /// A v4 fingerprint that can be compared with fresh reporter evidence.
    Fingerprinted(PersistedPanelFingerprintV4),
    /// A v4 record that never carried durable panel evidence.
    #[default]
    Anonymous,
}
...
pub(crate) struct PersistedPanelFingerprintV4(pub(crate) u64);
...
pub(crate) enum PersistedWindowTargetV5 {
    ...
    AwaitingLegacyEvidence(PersistedPanelIdentityV4),
}
